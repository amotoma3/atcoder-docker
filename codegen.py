...
class CppCodeGenerator:
    ...
    def generate_parameters(self) -> Dict[str, Any]:
        ...
        return dict(formal_arguments=self._formal_arguments(),
                    ...
                    declaration_part=self._declaration_part(),
    ...
    def _get_declaration_type(self, var: Variable):
        ...
        for d in range(var.dim_num()):
            if var.type == Type.int and d == 0:
                ctype = 'vl'
            elif var.type == Type.int and d == 1:
                ctype = 'vvl'
            else:
                ctype = 'vector<{}>'.format(ctype)
        return ctype
    ...
    def _generate_declaration(self, var: Variable):
        ...
        line = "{name} = {decl_type}{constructor};".format(
            name=var.name,
...
def main(args: CodeGenArgs) -> str:
    ...
    if(hasattr(args.constants, 'output_texts')):
        code_parameters['output_texts']='\n'.join(["// {output}".format(output=output) for output in args.constants.output_texts])
    return render(
        args.template,
